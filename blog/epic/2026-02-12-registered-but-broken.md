...
One wrinkle: after switching everything to direct JWKS, my server logs still show dozens of organizations actively hitting the `jwks.json` endpoint. And the traffic reveals something about the architecture that isn't documented anywhere: the JWKS fetch happens from each customer's own infrastructure, not from a central Epic service. The logs show requests from individual hospitals' IP ranges — Children's Hospital Colorado, Kaiser, Hennepin County Medical Center, Nationwide Children's — each independently reaching out. Organizations hosted by Epic come from Epic's hosting IPs. Some route through web security proxies like Forcepoint.

This explains the outbound traffic problem precisely. It's not a single Epic server that could be whitelisted once. Every customer's own signature validation servers need to make the outbound HTTPS request. If their network blocks it, the fetch fails. And since the fetches are still happening after switching to direct JWKS, it seems likely that Epic always attempts the URL fetch, and the direct keys serve as a fallback when the URL is unreachable — rather than replacing the fetch entirely.
